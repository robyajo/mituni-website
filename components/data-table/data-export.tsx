      ...
      if (getSelectedItems && selectedData && selectedData.length > 0) {
        ...
        if (sorting.length > 0) {
          ...
          if (!(sortField in sampleItem)) {
            console.warn(
              `Sort field "${sortField}" not found in data. Skipping sort.`
            );
            return sortedItems;
          }
      ...
      if (config?.allowExportNewColumns === false) {
        ...
        if (headers && headers.length > 0) {
          ...
          const existingHeaders = headers.filter(
            (header) =>
              allTableColumnIds.includes(header) &&
              visibleColumnIds.includes(header)
          );
          const newHeaders = headers.filter(
            (header) => !allTableColumnIds.includes(header)
          );
        ...
        {hasSelection && subRowsConfig?.enabled ? (
          ...
          <>
            {parentCount > 0 && enableCsv && (
              <DropdownMenuItem
                onClick={() => handleExportParents("csv")}
                disabled={isLoading}
              >
            ...
            {parentCount > 0 && enableExcel && (
              <DropdownMenuItem
                onClick={() => handleExportParents("excel")}
                disabled={isLoading}
              >
            ...
            {subrowCount > 0 && enableCsv && (
              <DropdownMenuItem
                onClick={() => handleExportSubrows("csv")}
                disabled={isLoading}
              >
            ...
            {subrowCount > 0 && enableExcel && (
              <DropdownMenuItem
                onClick={() => handleExportSubrows("excel")}
                disabled={isLoading}
              >
